...
const COLLECTIONS = [
  {
    label: "Prompted Cases",
    endpoint: "prompted_cases",
    outputDir: path.join(process.cwd(), "content", "prompt"),
    contentType: "prompt",
  },
  {
    label: "Prompted Projects",
    endpoint: "prompted_projects",
    outputDir: path.join(process.cwd(), "content", "projects"),
    contentType: "project",
  },
  {
    label: "Prompted Playground",
    endpoint: "prompted_playground",
    outputDir: path.join(process.cwd(), "content", "playground"),
    contentType: "playground",
  },
  {
    label: "Prompted Blog",
    endpoint: "prompted_blog",
...
async function syncPrompted() {
  for (const collection of COLLECTIONS) {
    await syncCollection(collection);
  }

  console.log("Done syncing Prompted by Adrianne!");
}

syncPrompted().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});